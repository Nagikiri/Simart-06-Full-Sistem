        ...
        <div class="relative z-10">
            <div class="flex items-center gap-3">
                ...
                @php
                    ...
                    $newUsersCount = \App\Models\User::whereMonth('created_at', now()->month)->count();
                @endphp
                <div class="flex -space-x-2">
                    @foreach($recentUsers as $u)
                        <div class="w-8 h-8 rounded-full bg-white/20 border-2 border-white/30 flex items-center justify-center text-[10px] font-bold text-white uppercase" title="{{ $u->name }}">
                            {{ substr($u->name, 0, 1) }}
                        </div>
                    @endforeach
                    @if($newUsersCount > 3)
                        <div class="w-8 h-8 rounded-full bg-white/15 border-2 border-white/20 flex items-center justify-center text-[10px] font-semibold text-white/70">+{{ $newUsersCount - 3 }}</div>
                    @endif
                </div>
                <div>
                    <p class="text-xs text-white/50">Warga baru bulan ini</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="w-full max-w-md">
            ...
            <div class="mt-8 rounded-xl p-4" style="background-color: #eceef0;">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-xs font-semibold mb-0.5" style="color: #191c1e;">Bantuan Sistem</p>
                        <p class="text-xs leading-relaxed" style="color: #3d4947;">Pastikan Email yang Anda daftarkan aktif untuk menerima notifikasi status pengajuan surat.</p>
                    </div>
                </div>
            </div>
